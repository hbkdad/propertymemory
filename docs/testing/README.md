...
Covers `properties`, `assets`, `extraction_jobs`, and `visual_annotations` as representative examples of the same `organization_id`-based RLS pattern (ADR 0002) used on every table: select isolation (including reading by a guessed id), insert isolation (a malicious insert is expected to raise a row-level-security policy violation, `SQLSTATE 42501`/`insufficient_privilege`), and update/delete isolation (a statement against a row RLS hides silently affects zero rows rather than erroring, so those are asserted by row count, not by expecting an exception). Verified the test can actually fail, not just always pass: deliberately weakened the `properties` select policy (and, separately, the `visual_annotations` one) to `using (true)` and confirmed the script caught each one and exited non-zero, then reverted.
